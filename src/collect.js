class Collect {
  ...
  async stand(message, userId, yourcard, dealercard, DECK, options) {
    ...
    if (dealervalue === 21 && dealercard.length === 2) {
      ...
      finalResult = {
        result: 'LOSE',
        method: `You lost <:emoji_5:990150803895484447> ${Number(options.betAmt).toLocaleString('en')}(dealer had blackjack).`,
        ycard: yourcard,
        dcard: dealercard
      };
      if (options.transition === 'edit') {
      ...
      if (options.hasInsurance == true) {
        ...
        if (options.isDoubleDown !== true) {
          if (yourvalue > 21) {
            finalResult = {
              result: 'LOSE',
              method: `You lost <:emoji_5:990150803895484447> ${Number(options.betAmt).toLocaleString('en')} (busted).`,
              ycard: yourcard,
              dcard: dealercard
            };
          } else if (yourvalue <= 21 && dealervalue < yourvalue) {
            finalResult = {
              result: 'WIN',
              method: `You won <:emoji_5:990150803895484447> ${Number(options.betAmt).toLocaleString('en')} (You had more points)`,
              ycard: yourcard,
              dcard: dealercard
            };
          } else if (dealervalue > yourvalue && dealervalue > 21 && yourvalue <= 21) {
            finalResult = {
              result: 'WIN',
              method: `You won <:emoji_5:990150803895484447> ${Number(options.betAmt).toLocaleString('en')} (dealer busted).`,
              ycard: yourcard,
              dcard: dealercard
            };
          } else if (dealervalue > yourvalue && dealervalue <= 21) {
            finalResult = {
              result: 'LOSE',
              method: `You lost <:emoji_5:990150803895484447> ${Number(options.betAmt).toLocaleString('en')} (dealer had more points).`,
              ycard: yourcard,
              dcard: dealercard
            };
          } else if (dealervalue === yourvalue) {
